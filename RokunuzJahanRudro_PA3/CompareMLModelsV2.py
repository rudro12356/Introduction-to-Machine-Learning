...
LDAmodelPrediction_1 = (LDAmodel.predict(X_test)).round()
LDAmodel.fit(X_train_2, y_train_2)
LDAmodelPrediction_2 = (LDAmodel.predict(X_test_2)).round()

actual = np.concatenate([y_test, y_test_2])
predicted = (np.concatenate(
    [LDAmodelPrediction_1, LDAmodelPrediction_2])).round()
...
print(f"\nAccurace score: " + str(accuracy_score(actual, predicted)))
print(f"\nConfusion matrix:\n " + str(confusion_matrix(actual, predicted)))
x = confusion_matrix(actual, predicted).sum()
print(f"Sum of Matrix: {x}")
...
rclfPrediction_1 = (rclf.predict(X_test)).flatten()
rclf.fit(X_train_2, y_train_2)
rclfPrediction_2 = (rclf.predict(X_test_2)).flatten()

actual = np.concatenate([y_test, y_test_2])
predicted = (np.concatenate([rclfPrediction_1, rclfPrediction_2])).round()

print("*******************10)RFC***********************************")
print(f"\nAccurace score: " + str(accuracy_score(actual, predicted)))
print(f"\nConfusion matrix:\n " + str(confusion_matrix(actual, predicted)))
y = confusion_matrix(actual, predicted).sum()
print(f"Sum of Matrix: {y}")

# Extra Tree classifier
eclf = ExtraTreesClassifier(n_estimators=100, random_state=0)
eclf.fit(X_train, y_train)
eclfPrediction_1 = (eclf.predict(X_test)).round()
eclf.fit(X_train_2, y_train_2)
eclfPrediction_2 = (eclf.predict(X_test_2)).round()

actual = np.concatenate([y_test, y_test_2])
predicted = (np.concatenate([eclfPrediction_1, eclfPrediction_2])).round()

print("*******************11)ETC***********************************")
print(f"\nAccurace score: " + str(accuracy_score(actual, predicted)))
print(f"\nConfusion matrix:\n " + str(confusion_matrix(actual, predicted)))
z = confusion_matrix(actual, predicted).sum()
print(f"Sum of Matrix: {z}")

# Neural Network
# initially had iterations as 100, increasing to 600 removed the convergence warning
nclf = MLPClassifier(random_state=0, max_iter=600).fit(X_train, y_train)
nclfPrediction_1 = (nclf.predict(X_test)).round()
nclf.fit(X_train_2, y_train_2)
nclfPrediction_2 = (nclf.predict(X_test_2)).round()

actual = np.concatenate([y_test, y_test_2])
predicted = np.concatenate([nclfPrediction_1, nclfPrediction_2]).round()
